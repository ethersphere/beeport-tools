#!/usr/bin/env node
/**
 * Parse sushi-bzz.sh matrix log (and compatible relay/lifi verbose lines).
 */
...
function parseLog(text) {
  ...
  while (i < lines.length) {
    ...
    const rec = {
      ...
      target_usd: usd,
      http,
      ok: okCompact || okVerboseRelay || okVerboseLifi || okVerboseSushi,
      error: "",
      bzz_out: "",
      in_usd: "",
      out_usd: "",
...
function writeMarkdown(rows, path, sourceLog, mode) {
  ...
  const title =
    mode.kind === "SUSHI"
      ? "# Sushi → BZZ — quote matrix (same-chain)"
      : "# Matrix — parsed log";
  lines.push(title);
  lines.push("");
  let modeNote;
  ...
  lines.push("");
  lines.push("| Chain | Token | Target USD | HTTP | BZZ out (est.) | Result |");
  lines.push("| --- | --- | ---: | ---: | --- | --- |");
  for (const r of rows) {
